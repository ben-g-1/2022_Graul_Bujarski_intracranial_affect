% function stim_table = generate_randomized_stim_table(fname)
...
% ------------------------------------------------------
basedir =  'C:\Users\bgrau\GitHub\git_ieeg_affect';
scriptsdir = fullfile(basedir, 'scripts');
addpath(scriptsdir)

% Load the table of stimuli and meta-data.  This will not change across
% subjects and is not randomized.
filedir = fullfile(scriptsdir, 'task');
...
for k = 1:2*npairs
    ...
    text(5.5, -.4, 'Extremely Positive', 'FontSize', 18)
    text(3.5, -.4, 'Neutral', 'FontSize', 18)
    plot([1 1], 0.3 * [-1 1], 'k', 'LineWidth', 3);
    plot([7 7], 0.3 * [-1 1], 'k', 'LineWidth', 3);
    plot([4 4], 0.3 * [-1 1], 'k', 'LineWidth', 3);
...
if bad_table_flag, error('Some Valence_mean values out of range'); end

bad_table_flag = any(stim_table.cue_observed_mean > 7 | stim_table.cue_observed_mean < 1);
if bad_table_flag, error('Some cue_observed_mean values out of range'); end

bad_table_flag = any(stim_table.cue_observed_std  < 0.4);
if bad_table_flag, warning('Some cues have very low std, < 0.4'); end
