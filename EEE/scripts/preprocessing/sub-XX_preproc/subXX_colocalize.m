...
subjectnum = 'XX';
sessionnum = 'XX';
canlabdir = ['//dartfs-hpc' filesep 'rc' filesep 'lab' filesep 'C' filesep 'CANlab'];
subjrawdir = [canlabdir filesep 'labdata' filesep 'data' filesep 'EEE' filesep 'ieeg' filesep 'raw' filesep 'sub-', num2str(subjectnum)];
anatdir = [subjrawdir filesep 'anat'];
fsdir = [subjrawdir filesep 'freesurfer'];

% cd(subjdir)

subjID           = ['sub-', subjectnum]; % for generating subject-specific filenames
mrfile           = [anatdir filesep ['EEE' subjID  '_T1.nii']]; % single file of a DICOM series or a nifti file
ctfile           = [anatdir filesep ['EEE', subjID, '_CT.nii']]; % single file of a DICOM series or a nifti file
eegfile          = [subjrawdir filesep ['ses-', num2str(sessionnum)] filesep ['EEE_', num2str(subjectnum), '_deidentified.EDF']]; % recording file (e.g., edf)
...
%% 5) write to file
cfg              = [];
cfg.filename     = [anatdir filesep ['EEE_', subjID, '_MR_acpc']];
cfg.filetype     = 'nifti';
cfg.parameter    = 'anatomy';
ft_volumewrite(cfg, mri_acpc);
ft_sourceplot([], mri_acpc); print([subjrawdir filesep 'anat' filesep ['EEE', subjID,  '_MR_acpc.png']], '-dpng'); close
...
if runfs
    ...
    mrifile          = [subjrawdir filesep subjID '_MR_acpc.nii'];
    tmpfile          = [subjrawdir filesep 'tmp.nii'];
    system(['export FREESURFER_HOME=' fshome '; ' ...
    'source $FREESURFER_HOME/SetUpFreeSurfer.sh; ' ...
    'mri_convert -c -oc 0 0 0 ' mrifile ' ' tmpfile '; ' ...
    'recon-all -i ' tmpfile ' -s ' 'freesurfer' ' -sd ' subjrawdir ' -all'])
end
%% 7) inspect the cortical surfaces
pial_lh          = ft_read_headshape([fsdir filesep 'surf' filesep 'lh.pial']);
pial_lh.coordsys = 'acpc';
ft_plot_mesh(pial_lh);
hold on;
pial_rh          = ft_read_headshape([fsdir filesep 'surf' filesep 'rh.pial']);
pial_rh.coordsys = 'acpc';
ft_plot_mesh(pial_rh);
lighting gouraud;
camlight;
print([subjrawdir filesep 'anat' filesep ['EEE', subjID, '_fsMR_acpc_pials.png']], '-dpng');

%% 8) import the FreeSurfer MR scan
fsmri_acpc       = ft_read_mri([fsdir filesep 'mri' filesep 'T1.mgz']);
...
print([subjrawdir filesep 'anat' filesep ['EEE', subjID, '_CT_acpc_f.png']], '-dpng');
...
%% 15) write to file
cfg              = [];
cfg.filename     = [subjrawdir filesep 'anat' filesep ['EEE', subjID, '_CT_acpc_f']];
cfg.filetype     = 'nifti';
cfg.parameter    = 'anatomy';
...
print([subjrawdir filesep 'anat' filesep ['EEE', subjID, '_elec_acpc_f.png']], '-dpng');

%% 20) write to file
save([subjrawdir filesep 'anat' filesep ['EEE', subjID, '_elec_acpc_f.mat']], 'elec_acpc_f');

%end %function colocalize