...
projdir = fullfile(basedir, 'EEE', 'Qualtrics');
filedir = fullfile(basedir, 'EEE', 'files');
ratedir = fullfile(projdir, 'ratings');
% scriptdir = fullfile(projdir, 'scripts', 'EVRTask');
rand_path = fullfile(ratedir, rand_settings);
...
for i = 1:(2*npairs)
     ...
     image_cue_vals = stim_table.cue_mean(i, 1) + cuesd * randn(nlines, 1); %  Normal distribution, with std = cuesd
...
stim_table.image_cue_values = image_cue_values;

stim_table.cue_observed_mean = cue_observed_mean;

stim_table.cue_observed_std = cue_observed_std;
...
%     figure('Color', [.5 .5 .5], 'Position', [200 200 800 250]);
    ...
    text(0.5, -.4, strneg,'color', 'k', 'FontSize', 18)
    text(6.5, -.4, strpos, 'color', 'k','FontSize', 18)
    text(3.6, -.4, 'Neutral', 'color', 'k', 'FontSize', 18)
    ...
    plot([stim_table.image_cue_values{k} stim_table.image_cue_values{k}]', [zeros(nlines, 1) ones(nlines, 1)]', 'k', 'LineWidth', 1.2);
    ...
    filename = append(num2str(k), "_", rand_settings, ".jpg");
...
set(gcf,'Units','pixels','Position', [200 200 800 250]);  %# Modify figure size
    ...
    filename = append(rand_settings, "_stats_vis.jpg");
