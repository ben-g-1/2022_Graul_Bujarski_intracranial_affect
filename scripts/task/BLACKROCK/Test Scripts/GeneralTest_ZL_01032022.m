...
if ( x == y )
    disp('Max compliance voltage not changed');
else
    disp('Max compliance voltage successfully changed');
end
...
if (fieldsExist) 
    ...
    if emptyfield
        disp('One or more fields are empty');
    else 
        disp('Device info read');
    end
else
    disp('One or more fields in Device Info Struct does not exist');
end
...
if (temp.moduleStatus(module) ~= devinfo.moduleStatus(module))
    disp('Module successfully disabled');
else 
    disp('Failed to disable module');
end

stimulator.enableModule(module);
temp = stimulator.deviceInfo();
if (temp.moduleStatus(module) == devinfo.moduleStatus(module))
    disp('Module successfully re-enabled');
else 
    disp('Failed to re-enable module');
end

% Set and get new waveforms
waveform_id = 1;
stimulator.disableStimulus(waveform_id);
disp('Waveform id 1 disabled');
...
disp('Waveform configuration read');
...
if (stimulus_max.frequency == stimulus_max2.frequency*2)
    disp('Max stimulus parameters successfully updated');
else
    disp('Failed to update stimulus parameters');
end
...
if (status == 4)
    disp('Stimulator is waiting for trigger');
else
    disp('Trigger not set');
end
