...
cfg              = [];
cfg.channel      = {'LSMA*', 'LFCA*', 'LFB*', 'LTHA*', 'LTA*'};
...
cfg.method       = 'cloud';
cfg.facealpha    = .25;
ft_sourceplot(cfg, freq_sel2, mesh_lha);
view([120 40]);
lighting gouraud;
camlight;
...
for t = 1:numel(freq_blc.time)
  ...
  cfg.facealpha    = .25;
  cfg.colorbar     = 'yes';
  ft_sourceplot(cfg, freq_blc2, mesh_lha);
  view([120 40]);
  lighting gouraud;
  camlight;
  text(nanmedian(mesh_lha.pos(:,1)),nanmedian(mesh_lha.pos(:,2)),min(mesh_lha.pos(:,3))-50,['time: ' num2str(round(freq_blc2.time,2))],'horizontalalignment','center');
