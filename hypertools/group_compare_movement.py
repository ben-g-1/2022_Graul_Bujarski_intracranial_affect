...
def main():
    ...
    # Agree vs. Disagreed trials
    agree_labels = getclasslabels(task_info, 'agree')
    agree_names = list(set(['Agree', 'Disagree']))

    columns = ["Subject ID", "Electrode", "Electrode Name", "Frequency", "Sample1", "Sample2"]
    ...
    for sub in subjects:
        for freq in freq_time:
            for chan, chan_name in channels.items():
                ...
                # Split to Agree (1) vs. Disagree (2) groups
                df1, df2 = group_agree(dataframe, agree_labels, ['Disagree', 'Agree'], [0, 1])
                avgdf1, avgdf2 = avgtrials([df1, df2])
                hyper_analyze([avgdf1, avgdf2])
                hyperplot([avgdf1, avgdf2], n_iter, ndims, agree_labels, agree_names)

                target_names = {
                    0: "Disagree",
                    1: "Agree"
                }
                pca_analyze(dataframe, agree_labels, target_names)

                # Append new stats dataframe row
                data = [sub, chan, chan_name, freq, avgdf1, avgdf2]
                stats_df = stats_df.append(pd.Series(data), ignore_index=True)

    two_sample_permtest(stats_df)
